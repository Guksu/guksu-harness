...
- **반복 설명을 줄인다.** 공통 작업 규칙은 `assets/harness-rules.md`에서 프로젝트 `docs/harness-rules.md`로 배포하고 다른 정의에는 포인터만 쓴다. 이 스킬에 규칙 전문을 복제하지 않는다.
...
```bash
node {이 스킬 경로}/scripts/harnessManager.mjs plan <프로젝트> --out <계획.json>
