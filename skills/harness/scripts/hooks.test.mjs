...
test('commit·push 예외(allowCommitPush) — 평시 커밋·푸시는 허용, 나머지 변경은 계속 차단한다', () => {
  const opt = { allowCommitPush: true };
  const allowed = [
    'git add src/auth.ts && git commit -m "feat: 로그인 검증 추가"',
    'git commit -m "$(cat <<\'EOF\'\nfix: 캐시 무효화 조건 수정\n\n- 만료 판정을 서버 시간 기준으로 변경\nEOF\n)"',
    'git push -u origin feat/login',
    'git push origin feat/login',
  ];
  for (const command of allowed) {
    assert.equal(
      judgeGitCommand(command, opt).blocked,
      false,
      `허용되어야 한다: ${JSON.stringify(command)}`,
    );
  }
  const blocked = [
    'git merge dev',
    ...
    'git commit -m "feat: x" && git rebase dev', // 복합 명령의 뒷부분도 검사한다
    'git switch -f main', // switch 파괴 플래그도 예외 모드에서 그대로 차단
  ];
  for (const command of blocked) {
    const verdict = judgeGitCommand(command, opt);
    assert.equal(verdict.blocked, true, `차단되어야 한다: ${JSON.stringify(command)}`);
    assert.equal(verdict.rule, 'mutation');
  }
  // 옵트인이 없으면(기본) 커밋·푸시도 여전히 차단이다
  assert.equal(judgeGitCommand('git commit -m "feat: x"').blocked, true);
...
test('commit·push 예외 — Claude 작성 표기가 든 커밋 메시지는 차단한다 (절대 규칙)', () => {
  ...
  for (const command of blocked) {
    const verdict = judgeGitCommand(command, opt);
    assert.equal(verdict.blocked, true, `차단되어야 한다: ${JSON.stringify(command)}`);
    assert.equal(verdict.rule, 'attribution');
  }
  // 표기 footer가 아닌 단순 "claude" 언급은 오탐하지 않는다
...
test('commit·push 예외 — 검사 불가 커밋 형태·force/delete push는 차단한다', () => {
  ...
  for (const command of commitBlocked) {
    const verdict = judgeGitCommand(command, opt);
    assert.equal(verdict.blocked, true, `차단되어야 한다: ${JSON.stringify(command)}`);
    assert.equal(verdict.rule, 'commit-flags');
  }
  const pushBlocked = [
    ...
    'git push "-f" origin feat/x', // 셸이 따옴표를 벗겨 전달한다
    'git push -fu origin feat/x', // 번들 단축 옵션 (-f + -u)
  ];
  for (const command of pushBlocked) {
    const verdict = judgeGitCommand(command, opt);
    assert.equal(verdict.blocked, true, `차단되어야 한다: ${JSON.stringify(command)}`);
...
test('git 훅 CLI — allowCommitPush 옵트인으로 커밋이 열리고, 표기 커밋·설정 오류는 차단된다', async () => {
  ...
  assert.equal(off.status, 2, 'config가 없으면 커밋은 기본 차단이다');

  const on = await runGitMutationCli({
  ...
  assert.ok(attribution.stderr.includes('Claude'));

  const broken = await runGitMutationCli({
    config: '{ "allowCommitPush": true,, }', // 파싱 실패
    command: 'git commit -m "feat: x"',
  });
