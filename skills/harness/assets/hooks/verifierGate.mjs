...
// (2) 실패가 남았는데 안전장치 도달 = 계속하지 않고 보고 후 종료 지시,
// (3) 실패 + 여력 있음 = 차단하고 계속.
export const decide = ({ config, iterations, tokensUsed, failures }) => {
...
if (isDirectRun) {
  ...
  const iterations = state[input.session_id] ?? 0;

  const failures = runChecks({ checks: config.checks ?? [], cwd: input.cwd });
  const decision = decide({ config, iterations, tokensUsed, failures });
  if (decision.action === 'allow') process.exit(0);

  state[input.session_id] = iterations + 1;
