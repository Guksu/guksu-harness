...
`docs/harness-rules.md`는 `init`이 한 번 만들고 그 뒤로는 팀 파일이다. 코어 정책은 `.agents/harness-core-rules.md`에 있고, 팀 규칙이 코어보다 우선한다.

```markdown
## 팀 규칙

1. **커밋 메시지는 한국어 Conventional Commits로 쓴다.** 예: `feat: 로그인 화면 추가`
2. **기록 정책:** 작업 기록은 PR 단위가 아니라 스프린트 단위로 `docs/history/`에 하나만 남긴다.
```

코어 규칙을 바꾸고 싶으면 "우리 팀에서는 …"으로 적는다. 코어 사본을 직접 고치지 않는다. 스킬과 에이전트는 두 파일을 함께 읽는다.
