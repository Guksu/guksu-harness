---
name: fe-predeploy
description: "프론트엔드 배포 전 점검 — 정적 게이트(빌드·린트·테스트·정적 스캔) → 실제 브라우저 런타임 계측(버튼 연타 시 API 1회·메모리 누수·네트워크 경계) → 성능 측정(Core Web Vitals) → 시각·UX 스윕으로 배포 가능 여부를 판정하고 docs/predeploy/ 체크리스트로 기록한다(퓨어 JS·React·Next.js). (1) '배포 전 점검/프론트 QA 돌려줘/배포해도 돼?/릴리스 체크' 요청 시, (2) 프론트엔드 작업 완료 후 배포·릴리스를 앞뒀을 때, (3) '중복클릭 확인/메모리 누수 검사/성능 측정/웹바이탈 확인' 등 개별 항목 요청 시, (4) '재점검/실패 항목만 다시/기준치 조정/점검 결과 갱신' 등 후속 요청 시 반드시 이 스킬을 사용할 것. 발견된 버그의 수정 작업 자체는 대상이 아니다 — 점검이 수정을 겸하지 않는다."
---
...
1. **체크리스트 기록** — `docs/predeploy/{YYYY-MM-DD}-{slug}.md` (템플릿: 프로젝트 `docs/templates/predeploy.md`, 없으면 docs 스킬 절차로 복사). 전 항목의 판정·근거 수치·skip 사유를 남긴다. 재점검 시 새 파일이 아니라 기존 문서의 판정을 갱신하고 재실행 이력에 추가한다.
2. **종합 판정** — blocker fail 0건이면 "배포 가능", 있으면 "배포 불가 + 목록". warn fail은 판정을 막지 않되 사용자 검토로 올린다.
3. **보고** — report 스킬 사용 환경이면 HTML 보고서의 "검증 결과"에 반영하고, 채팅에는 요약(판정 + fail/skip 수 + 경로)만. 실패 항목은 수정 제안까지만 한다(원칙 1).
