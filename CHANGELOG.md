...
- `npx guksu-harness …`가 아무것도 하지 않고 종료 코드 0으로 끝나던 결함. npx·npm은 `node_modules/.bin/guksu-harness` 심볼릭 링크로 실행하는데, 명령 파일의 직접 실행 판정이 링크 경로와 실제 경로를 문자열로 비교해 어긋났다. 양쪽을 realpath로 맞춰 비교한다(훅 스크립트에는 이미 있던 보호). 링크 경유 실행 테스트 추가(143→144). 4.2.0은 저장소 안에서 `node bin/…`로만 검증했고 배포본을 npx로 돌려 보지 않아 놓쳤다.
...
- `harnessManager.mjs`: 번들·설치 버전, 파일 변경 상태, 훅 등록·설정 진단. 공통 파일의 해시 추적, 기존 설정 병합, 선택 적용, 사용자 수정 충돌 보존, 백업 복원.
- `decideRelease.mjs`: 필수 검사 누락을 판정 보류로 분류하는 배포 판정기.
- 실제 CLI 설치·업데이트·제거·복원, 계획 변경 거부, 연속 Stop 이벤트와 배포 판정 회귀 테스트.
- 읽기 전용 요청·작은 수정·업로드 범위를 검토하는 모델 실행 평가 시나리오. 별도 모델 실행 평가는 이번 검증에 포함하지 않았다.

### Fixed

- 종료 훅이 첫 실패 뒤 `stop_hook_active`만 보고 검사를 건너뛰던 문제. 이후 Stop도 재검사하고 한도·막힘 시 중단 보고 후 종료한다.
- 기본 구성에서 협업 진행표를 만들라는 상충 지침과 불필요한 협업 템플릿 경고.
- 필수 검사 skip을 배포 가능으로 오판할 수 있던 최종 판정 규칙.

### Changed

- 점검·분석은 읽기 전용, 수정·동기화는 요청 범위에서 실행한다. 이미 받은 구현 승인을 반복해서 묻지 않는다.
- 일반 개발의 테스트 재시도는 별도 루프 승인·명세 없이 진행한다. 반복 실행과 역할 분리를 따로 판단한다.
- 브랜치 전략·작성자 표기·기록은 프로젝트 정책을 따른다. 작은 수정마다 별도 기록을 만들지 않는다.
- README를 설치·첫 요청·문제 해결 중심으로 축소하고 관리 도구·보호 장치 상세는 참조 문서로 이동했다.
- **업데이트 주의:** Claude 작성자 표기 차단은 이제 `blockAttribution: true`인 프로젝트만 적용한다. 기존 제한을 유지하려면 훅 업데이트 전에 설정한다. 커밋·푸시 허용 설정은 자동 변경하지 않는다.
- 검증 훅 상태는 세션별 파일에 저장한다. `maxTokens`는 선택적 세션 누적 근사치이며 Stop 시점에 확인한다. 설정했는데 transcript를 읽지 못하면 중단 사유를 보고한다.

## [2.2.0] - 2026-09-04

독자 기준 출력 릴리스. Anthropic 커뮤니티 플러그인 `eli5` 스킬(10줄짜리 출력 정책 — "아무것도 모르는 사람에게, 그림은 크게 글은 적게")의 원리를 절대 규칙 7에 옮겼다. v1.17.0의 규칙 7은 *문장 단위*(용어 풀이·한 문장 하나의 내용)였고, 이번에 *구조와 형식*(결론 → 할 일 → 근거, 실패 먼저, 경로·수치는 표로, 글보다 표·다이어그램)을 더했다. 규칙 수는 늘리지 않았다("규칙은 늘리지 않는다" 원칙).

### Added

- **`references/plain-output.md` (신규, harness 스킬 10번째 reference)** — 규칙 7 상세: 독자 모델 3종(요청한 사용자·다음 세션·코드를 모르는 이해관계자 → 최소공배수 "대화를 못 봤고 코드를 모른다"), 결론 → 할 일 → 근거 구조, 문장 규칙, 용어 처리, 코드·경로·수치 배치, 형식 선택표(표/배지/다이어그램/코드 블록), 매체별 형식 7종(채팅·작업 기록·인계·배포 점검·리뷰·설계 문답 질문·구축 계획), 안티패턴 7, Before/After 2, 점검 질문 6. 설계 문답의 질문 규칙은 `design-dialogue.md` §5가 정본이며 이 문서는 보고·기록으로 확장한다
- **validateHarness 규칙 파일 검사** — 프로젝트 `docs/harness-rules.md`가 없으면 warn, 있는데 규칙 수가 플러그인 정본보다 적으면 "구버전" warn. 정본이 개정될 때(이번처럼) 기존 하네스가 조용히 뒤처지지 않게 한다. 회귀 테스트 3종 추가(74→77)

### Changed

- **절대 규칙 7 확장** (`assets/harness-rules.md`·SKILL.md·README 동기화) — 독자 가정("이 대화를 보지 못했고 코드를 모르는 사람"), 순서(결론 → 사용자가 할 일 → 근거, 실패·미검증은 결론에 먼저), 경로·명령·수치는 산문이 아니라 표·목록·코드 블록에, 항목 비교·판정·흐름은 표·배지·다이어그램으로
- harness SKILL.md — Phase 0 감사 항목에 규칙 파일 누락·구버전, Phase 2-1 규칙 파일 갱신 절차(구버전 경고 시 사용자 확인 후 누락 규칙만 추가 — 프로젝트가 손본 문구 보존), Phase 3-6 출력 가독성 점검, 체크리스트·참조 표 갱신. 참조 표의 `context-economy.md` 행이 "절대 규칙 7"을 가리키던 오류를 "핵심 원칙 4"로, `design-dialogue.md` §5의 "절대 규칙 8"을 "7"로 정정(v2.0.0에서 규칙 번호가 이동했는데 두 곳이 남아 있었다)
- 템플릿 3종 안내 주석에 규칙 7 반영 — history(개요 첫 문장은 결론, 확인 필요·후속은 동사 항목), handoff(용어 풀이·행동 문장), predeploy(fail은 "사용자에게 어떤 일이 생기는가"로)
- 스킬 보고 절차 — history(독자 기준 기록), handoff(용어 풀이·행동 문장), fe-predeploy(판정 첫 문장·fail 풀이), fe-craft(Why에 원칙 이름 + 사용자에게 보이는 증상)
- orchestrator-template 종료 절차·agent-design 입출력 계약에 "사용자 대면 보고: 결론 → 할 일 → 근거" 한 줄 추가
- **README를 규칙 7 기준으로 다듬음** — 결론부터 나오는 "30초 요약"(할 일 3단계 + 상황별 읽기 안내) 신설, 용어 사전에 정의 없이 쓰이던 8개(CLAUDE.md·description·실행 모드·티어·TDD·경계면 QA·git-flow·blocker) 추가, §1 산출물 표에 규칙 파일 행, §4-6 "여기까지 됐다면", §5 fe-predeploy 실패 항목 풀이·fe-craft Why 풀이·validateHarness 검사 항목, §6 사이클에 "보고" 단계와 종료 보고 예시, §7 규칙 7 행, §8-5 신설, §12 갱신

## [2.1.0] - 2026-08-29

**fe-skills 라이브러리 접목** — 프론트엔드 작업의 네 시점(설계 판정 → 구현 → 제작 품질 → 배포 게이트) 중 이 플러그인이 담당하지 않는 앞의 둘을 [`Guksu/fe-skills`](https://github.com/Guksu/fe-skills)로 연결했다. **플러그인 설치가 아니라 라이브러리 사용이다** — npm에서 쓸 패키지만 받듯 필요한 스킬 하나만 가져오고, 사용자가 요청하지 않아도 UI 패턴 작업이 생기면 먼저 조회한다. 상시 로딩 비용 0.

연결은 원래 한 방향뿐이었다 — fe-skills(소비자)의 CLAUDE.md는 `fe-craft`·`fe-predeploy`를 가리키는데, 하네스(생산자)는 fe-ui·fe-system의 존재를 몰라서 **다른** 프로젝트에 프론트엔드 하네스를 구축할 때 그 둘을 배선하지 못했다.

### Added

- **`scripts/feSkills.mjs` (신규) — fe-skills 라이브러리 클라이언트.** 설치 없이 필요한 스킬만 가져온다.
  - `find <요청 문장>` — 사용자 표현("바텀시트로 메뉴 고르게 해줘")으로 후보를 순위대로. slug 정확 일치 > slug 토큰 > 설명 포함 순이며, 조사가 붙은 한글도 매칭한다
  - `get <slug> --into <대상>` — SKILL.md 경로를 출력하고 정본 코드(`assets/`) 또는 판정 근거(`references/`)를 프로젝트로 복사. 덮어쓴 파일을 보고하고 출처(MIT)를 표기한다
  - `list [--json]` — 전체 31종 카탈로그
  - 캐시는 얕은 클론(`$XDG_CACHE_HOME/guksu-harness/fe-skills`). **갱신에 실패해도 기존 캐시로 진행한다** — 낡은 라이브러리가 없는 라이브러리보다 낫다. 캐시도 네트워크도 없으면 exit 3으로 알리되, 호출부는 이를 "직접 구현"으로 넘어가는 신호로만 쓴다(작업은 멈추지 않는다)
- **`references/frontend-domain.md` (신규)** — 프론트엔드 도메인일 때만 읽는 배선 문서. 네 시점, 라이브러리 사용법, **사용자 요청 없이 발동시키는 방법**, 출처 기록. 분리 기준은 분기다(skill-authoring) — 프론트엔드는 분기이므로 references로 내렸고, 그만큼 SKILL.md 본문이 짧아졌다
- **자동 발동 규칙** — Phase 2에서 `feSkills.mjs`를 프로젝트 `.claude/scripts/`로 복사하고(훅과 같은 이유로 그대로 복사 — 플러그인 미설치 세션에서도 동작한다), 도메인 스킬·developer 에이전트 정의에 "UI 패턴 구현·화면 설계 전 라이브러리 먼저 조회"를 **완료 기준과 함께** 넣는다. 완료 기준("패턴 구현 시작 전에 `find`를 실행했고, 후보가 있었다면 그 SKILL.md를 읽었다")을 빼면 바쁜 실행에서 건너뛰어진다
- **경계 정의: `fe-system` ↔ Phase 1 설계 문답** — 대상이 달라 경쟁하지 않는다. 설계 문답의 대상은 *하네스 자체*(어떤 역할·스킬·안전장치를 만들 것인가)이고 `fe-system`의 대상은 *기능의 구현 설계*(이 목록 화면의 상태를 어디에 둘 것인가)다. 따라서 배선 지점은 Phase 1이 아니라 **구축된 하네스가 실제 요청을 받는 자리**(도메인 스킬·에이전트 정의)다
- **회귀 테스트 5종** — frontmatter 파싱(멀티라인 인용 포함), 카탈로그 구성(두 플러그인·payload 구분·깨진 스킬 제외), 검색 랭킹(slug 일치·한글 조사·무매칭 0건), 정본 복사(덮어쓰기 보고), 캐시 폴백(갱신 실패 시 기존 캐시·둘 다 없으면 실패)

### Changed

- **`fe-craft`에 라이브러리 조회 절 추가** — 이름 붙은 UI 패턴은 직접 짜서 비평하기 전에 라이브러리를 조회한다. 모션·접근성·`prefers-reduced-motion`이 정리된 코드에서 출발하면 비평 루프가 줄어든다
- **Phase 2 step 6의 프론트엔드 문단 → 포인터 한 줄** — 내용은 `frontend-domain.md`로 이동
- **`verifierGate`의 `maxTokens` 의미 명시 + 권장 기본값 500,000 → 20,000,000** — 판정 로직은 그대로다(세션 transcript 누적 합계). 문서가 이 값을 "루프 1회분 예산"처럼 읽히게 써서 실사용에서 오설정을 유발했다: 매 턴의 `input_tokens`에 대화 전체가 다시 들어가므로 합계는 세션 길이에 초선형이고, 500,000은 정상 작업 중에 매 턴 발동한다(실제로 `Guksu/fe-skills`가 20,000,000으로 올려 우회했다). **"이 세션을 여기서 끊는다"는 상한**으로 잡으라고 명시하고, 반복을 실제로 제어하는 것은 `maxIterations`·`stuckAfter`임을 `loop` 스킬 Phase 2에 분리해 적었다
