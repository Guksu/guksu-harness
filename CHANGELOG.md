...
**fe-skills 접목** — 프론트엔드 작업의 네 시점(설계 판정 → 구현 → 제작 품질 → 배포 게이트) 중 이 플러그인이 담당하지 않는 앞의 둘을 [`Guksu/fe-skills`](https://github.com/Guksu/fe-skills)의 `fe-system`·`fe-ui`로 연결했다. **흡수가 아니라 포인터 연동이다** — `fe-ui` 29종의 description을 이 플러그인에 넣으면 백엔드·CLI 프로젝트에서도 매 턴 로딩되므로, 프론트엔드 프로젝트만 설치하는 편이 옳다(v2.0.0의 상시 로딩 규율).
...
- **`references/frontend-domain.md` (신규)** — 프론트엔드 도메인일 때만 읽는 배선 문서. 네 시점과 담당 스킬, fe-skills 설치 안내, 배선 위치, 템플릿 범위를 정한다. 분리 기준은 분기다(skill-authoring) — 프론트엔드는 분기이므로 본문이 아니라 references로 내렸고, 그만큼 SKILL.md 본문이 짧아졌다
- **경계 정의: `fe-system` ↔ Phase 1 설계 문답** — 다루는 대상이 달라 경쟁하지 않는다. 설계 문답의 대상은 *하네스 자체*(어떤 역할·스킬·안전장치를 만들 것인가)이고 `fe-system`의 대상은 *기능의 구현 설계*(이 목록 화면의 상태를 어디에 둘 것인가)다. 따라서 배선 지점은 Phase 1이 아니라 **구축된 하네스가 실제 요청을 받는 자리**(도메인 스킬·developer 에이전트 정의)다
- **README §5-1 "프론트엔드는 마켓플레이스가 하나 더 있습니다"** — 네 시점 표, 설치 명령, 합치지 않은 이유. FAQ에 같은 질문 추가
- **산출물 체크리스트 항목** — 설치 여부를 확인받고 **설치한 것만** 배선한다. 없는 스킬을 가리키는 포인터는 dead link이고, 에이전트가 찾다 실패하면 그 정의 전체의 신뢰가 떨어진다

### Changed

- **`fe-craft`에 fe-ui 연계 절 추가** — 이름 붙은 UI 패턴(바텀시트·핀치줌 등)은 직접 짜서 비평하기 전에 fe-ui에 정본이 있는지 먼저 찾는다. 모션·접근성·`prefers-reduced-motion`이 정리된 코드에서 출발하면 비평 루프가 줄어든다. 설치되지 않았으면 무시하도록 조건부로 서술
