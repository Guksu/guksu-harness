...
현재 버전 **v1.17.0** · 스킬 11종 · 안전 훅 4종 · 회귀 테스트 75종 · MIT
...
**질문은 한 번으로 끝나지 않습니다.** 신규 구축은 파일을 만들기 전에 **설계 문답**을 여러 라운드(최소 3회) 반복합니다 — 목표·범위 → 도메인·데이터 → 실행 방식·안전장치(보호 브랜치, 커밋 권한 등) → 종합 확인 순서로, 매 라운드 지금까지의 결정을 다시 보여주고 모순이 없는지 점검합니다. 질문은 쉬운 말로 나옵니다(전문용어는 한 줄 풀이가 붙습니다). 잘 모르겠으면 "추천대로 해줘"라고 답해도 됩니다 — 모든 질문에 추천안이 준비되어 있습니다.
