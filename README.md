...
현재 버전 **v1.14.0** · 스킬 10종 · 안전 훅 4종 · 회귀 테스트 73종 · MIT
...
- [5. 스킬 10종 — 무엇을 언제 쓰나](#5-스킬-10종--무엇을-언제-쓰나)
...
`/plugin` 을 입력해 `guksu-harness`가 목록에 보이면 성공입니다. 또는 `/guksu-harness:` 까지만 입력하면 스킬 10종이 자동완성으로 뜹니다.
...
| **1** | 설계 | 프로젝트 도메인을 분석하고, 실행 모드(§8-2 사다리)를 고르고, 에이전트를 몇 개로 나눌지 판단 | **역할 분리안·실행 모드에 동의 여부 답변** |
| **2** | 구축 | 에이전트 정의·스킬·오케스트레이터 파일 생성, 훅과 권한 설정, 문서 템플릿 6종 배포 | 보호 브랜치 목록·커밋 권한 옵트인 등 **확인 질문에 응답** |
...
```bash
ls .claude/agents/       # 역할별 정의 파일들
ls .claude/hooks/        # 훅 4종 + config
ls docs/templates/       # 문서 템플릿 6종
...
**번들 템플릿 6종** — worklog · retro · handoff · loop-spec · digest · report.html. 하네스 구축 시 프로젝트의 `docs/templates/`로 복사되고, **그 복사본이 그 프로젝트의 단일 출처**가 됩니다(프로젝트마다 양식을 조정할 수 있게).
...
2. **실행** — 오케스트레이터가 선택된 실행 모드로 진행합니다. 코드 생성은 TDD로, 모듈이 완성될 때마다 경계면 QA를 돌립니다
3. **기록** — 작업한 에이전트가 `docs` 워크로그를 남기고, 종료 시 `report`가 HTML 보고서를 만듭니다(채팅에는 요약만)
4. **진화** — 종료 시 `retro`를 **제안**합니다. 강요하지 않고, 승인된 개선안만 반영합니다
...
생성되는 모든 하네스의 에이전트 정의와 오케스트레이터에 내장됩니다.
...
프로젝트/
├── .claude/
│   ├── agents/{planner,developer,qa,...}.md   # 에이전트 정의 (빌트인 타입이라도 파일로 명시)
│   ├── skills/{도메인}-orchestrator/           # 오케스트레이터 (실행 모드·데이터 흐름·에러 처리)
...
├── docs/
│   ├── templates/                             # 공통 템플릿 6종 (이 사본이 프로젝트의 단일 출처)
