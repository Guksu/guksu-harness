...
현재 버전 **v2.2.0** · 스킬 9종 · 안전 훅 4종 · 회귀 테스트 77종 · MIT

---

## 요약

**한 문장을 입력하면, 그 프로젝트에 Claude가 따를 규칙·절차·차단기·문서 양식이 생깁니다.** 이후 Claude는 브랜치를 확인하고 → 작업하고 → 기록하고 → 결론부터 짧게 보고하는 사이클로 일합니다. 커밋·시크릿 읽기·보호 브랜치 편집 같은 위험한 일은 부탁이 아니라 프로그램(훅)이 막습니다.

**당신이 할 일은 셋뿐입니다.**

| 순서 | 할 일 | 입력 |
|---|---|---|
| 1 | 설치 | `/plugin marketplace add Guksu/guksu-harness` → `/plugin install guksu-harness@guksu-harness` |
| 2 | 구축 | `/guksu-harness:harness 이 프로젝트에 하네스 구축해줘` |
| 3 | 설계 문답에 답하기 | 목표·범위·실행 방식·안전장치를 몇 라운드 묻습니다. 모르겠으면 "추천대로 해줘" |

**어디부터 읽을까**

| 지금 상황 | 읽을 곳 |
|---|---|
| 처음이라 뭔지 모르겠다 | §1 → §2 → §3 → §4 (10분) |
| 설치했고 스킬을 골라 쓰고 싶다 | §4-2 → §5 |
| 뭔가 막혔다 | §11 → §10 |
| 직접 설계·커스터마이즈하고 싶다 | §7 → §8 → §9 → §12 |
...
- [5. 스킬 9종 — 무엇을 언제 쓰나](#5-스킬-9종--무엇을-언제-쓰나)
...
| **규칙 파일** | 모든 역할이 따르는 절대 규칙 7종 (git·TDD·기록·단일 출처·QA·시크릿·보고 방식) | `docs/harness-rules.md` |
| **문서 템플릿** | 작업 기록·인계·회고 문서의 공통 양식 | `docs/templates/` |

이후 그 프로젝트에서 하는 모든 작업은 **브랜치 확인 → 작업 실행 → 기록**이라는 일정한 사이클로 돌아갑니다. 매번 강제되는 의례는 이 셋뿐이고, 회고 같은 나머지는 필요할 때만 붙습니다. 끝날 때의 보고는 항상 **결론 → 당신이 할 일 → 근거** 순서로, 대화를 보지 못한 사람도 이해할 수 있게 씁니다.
...
| **설계 문답** | 하네스를 만들기 전에 사용자와 여러 라운드의 질문·답변으로 설계를 확정하는 절차. 결과는 설계서(`docs/design/`)로 남습니다 | 착공 전 설계 미팅 |
| **프런티어** | 설계 문답에서 지금 답할 수 있는 질문들의 집합. 전제가 확정된 질문만 묻고, 아직 안 정해진 답에 의존하는 질문은 다음 라운드로 미룹니다 | 퍼즐에서 지금 맞출 수 있는 조각들 |
| **공개 경계 (seam)** | 테스트를 거는 위치 — 구현 내부가 아니라 호출자가 쓰는 인터페이스. 어느 경계를 지킬지는 설계 문답에서 합의합니다 | 완제품 검사 지점 |
| **작업 기록 (history)** | 무엇을 왜 어떻게 했는지 남기는 문서. PR 하나당 하나 | 업무 일지 |
| **작업 산출물** | 역할 간에 주고받는 중간 파일(`_workspace/`). 커밋되지 않고 작업이 끝나면 버려집니다 | 회의 중 화이트보드 |
| **보호 브랜치** | 직접 편집하면 안 되는 브랜치 (기본값: `main`, `master`) | 출입 통제 구역 |
| **Workflow** | 여러 에이전트의 실행 흐름을 자바스크립트 코드로 짜서 돌리는 실행 방식 | 자동화 스크립트 |
| **CLAUDE.md** | 프로젝트 루트에 두는 Claude 지침 파일. 세션 시작 시 자동으로 읽힙니다. 하네스는 여기에 포인터와 변경 이력만 남깁니다 | 현관 안내판 |
| **description** | 스킬 파일 머리말의 한 줄 소개. 사용자 요청과 이 문장이 맞으면 스킬이 자동 발동합니다 | 호출 신호 |
| **실행 모드** | 작업을 어떤 규모로 돌릴지 — 직접 실행부터 에이전트 팀까지 5단계. 싼 쪽부터 시작합니다 | 기어 단수 |
| **티어 (라이트/풀)** | 실행 모드에 따라 만들어지는 파일의 규모. 라이트는 스킬·훅·규칙만, 풀은 에이전트 정의·오케스트레이터까지 | 기본형 / 풀옵션 |
| **TDD** | 테스트를 먼저 쓰고 그 테스트를 통과시키는 순서로 코드를 만드는 방식 | 정답지 먼저, 풀이 나중 |
| **경계면 QA** | 데이터를 만드는 쪽(API)과 쓰는 쪽(화면)을 같이 읽어 모양이 맞는지 비교하는 검증 | 배관 이음새 점검 |
| **git-flow** | `main`(배포) ← `dev`(통합) ← `feat/*`(작업) 순으로 브랜치를 나누는 운영 방식 | 3단 결재선 |
| **blocker** | 배포 전 점검에서 "이건 있으면 배포 불가"인 등급. 나머지는 warn(검토 권고) | 빨간불 / 노란불 |
...
`/plugin` 을 입력해 `guksu-harness`가 목록에 보이면 성공입니다. 또는 `/guksu-harness:` 까지만 입력하면 스킬 9종이 자동완성으로 뜹니다.
...
| 작업 기록 남기기 | `/guksu-harness:history 이번 작업 기록해줘` |
...
| **1** | 설계 문답 → 설계 | 프로젝트를 분석한 뒤 **사용자와 반복 문답으로 설계부터 확정** — 매 라운드 이전 답변을 종합해 보여주고 다음 질문 2~4개를 던지며, 승인된 설계서를 `docs/design/`에 남긴다. 이어서 실행 모드(§8-2 사다리)와 **티어(라이트/풀)** 를 고르고, 풀 티어면 에이전트를 몇 개로 나눌지 판단 | **라운드별 질문에 답하고, 최종 설계 요약을 승인** |
| **2** | 구축 | 티어에 맞는 산출물 생성 — 라이트: 도메인 스킬 + 훅·권한 + 규칙 파일 + 템플릿 2종 / 풀: 에이전트 정의·오케스트레이터·템플릿 4종까지 | 보호 브랜치 목록·커밋 권한 옵트인 등 **확인 질문에 응답** |
| **3** | 검증 | `validateHarness.mjs` 재실행(구조), 비슷한 요청으로 트리거 테스트, 컨텍스트 낭비 점검 | 결과 확인 |
| **4** | 등록·진화 | `CLAUDE.md`에 하네스 포인터와 변경 이력 기록 | — |

**질문은 한 번으로 끝나지 않습니다.** 신규 구축은 파일을 만들기 전에 **설계 문답**을 반복합니다 — 목표·범위 → 도메인·데이터 → 실행 방식·안전장치(보호 브랜치, 커밋 권한 등) → 종합 확인 순서로, 매 라운드 지금까지의 결정을 다시 보여주고 모순이 없는지 점검합니다. **끝나는 시점은 라운드 수가 아니라 수렴이 정합니다** — 미해결 질문 0 · 모순 0 · 몰래 깔린 가정 0 · 사용자 승인, 이 넷이 채워지면 끝입니다. 라운드 수를 채우려고 이미 나온 답을 다시 묻지 않습니다. 각 라운드는 **프런티어**(지금 답할 수 있는 질문)만 묻고, 파일·스택 같은 **사실은 묻는 대신 직접 조사**합니다 — 사용자에게는 결정만 옵니다. 질문은 쉬운 말로 나옵니다(전문용어는 한 줄 풀이가 붙습니다). 잘 모르겠으면 "추천대로 해줘"라고 답해도 됩니다 — 모든 질문에 추천안이 준비되어 있습니다. 문답이 겉돌면 두 가지 탈출구가 있습니다: 실행해 봐야 아는 결정은 1회용 **프로토타입**으로 직접 확인하고, 답을 제3자(팀 리드·인프라 담당 등)가 아는 결정은 그 사람에게 전달할 **질문지**로 만들어 드립니다. 종료 전에는 확인 없이 깔린 가정이 없는지 검사합니다 — 몰래 깔린 가정은 나중에 재작업으로 돌아오기 때문입니다.

### 4-4. 완료 후 확인할 것

```bash
ls .claude/hooks/        # 훅 + config
ls docs/design/          # 설계 문답으로 승인된 설계서
ls docs/templates/       # 문서 템플릿 (라이트: 2종, 풀: 4종 + 설계서)
...
- 평소처럼 작업을 요청하면 됩니다. 스킬을 직접 고르고 싶을 때는 §4-2의 문장을, 각 스킬이 무엇을 하는지는 §5를 보세요
- 뭔가 막히면 §11 문제 해결 표부터 보세요
...
| [`history`](#history--작업-기록) | 완료된 작업을 PR 단위로 한 문서에 기록 | `docs/history/` |
...
- 신규 구축은 **설계 문답부터** 합니다 — 파일을 만들기 전에 사용자와의 문답으로 설계를 확정하고, 승인된 설계서(`docs/design/`)가 이후 구축의 단일 출처가 됩니다.
...
- 구조 검사기 `validateHarness.mjs`가 스킬 파일 머리말(frontmatter) 누락·깨진 참조 링크·이름 불일치·버전 정합성·규칙 파일 누락이나 구버전을 자동으로 잡습니다. 플러그인을 업데이트해 규칙이 바뀌면 "점검"에서 구버전 경고로 알려줍니다.
...
**언제 쓰나** — 작업이 끝났을 때, 커밋·PR 직전에. 또는 "기록해줘 / 문서화해줘"라고 요청했을 때.

**PR 하나 = 문서 하나.** 완료된 작업의 보존 기록은 `docs/history/`의 문서 하나뿐입니다. 역할(기획·구현·QA)마다 각자 기록을 남기면 같은 작업이 여러 문서에 흩어져, 나중에 읽는 사람이 어느 것이 전체인지 알 수 없습니다.

**작업 산출물과 기록은 다른 곳에 둡니다** — 수명이 다르기 때문입니다.

| | 위치 | 수명 | 무엇 |
|---|---|---|---|
| 작업 산출물 | `_workspace/` (gitignore) | 이번 작업 동안 | 역할 간 인계물 — 계획서·구현 노트·QA 리포트 |
| 기록 | `docs/history/` (커밋) | 영구 | 완료된 작업 1건의 정본. 산출물을 근거로 마지막에 종합 |

**양식은 5부분** — 1. 개요 / 2. 작업 내용 / 3. 검증 결과 / 4. 확인 필요·후속 / 5. 주의사항

양식이 통일되어야 하는 이유가 있습니다. 기록의 독자는 사람만이 아니라 **다음 세션의 Claude**입니다. 형식이 제각각이면 읽는 쪽이 매번 구조를 파악해야 하고, 그만큼 컨텍스트가 낭비됩니다. 특히 "3. 검증 결과"에는 실제로 실행한 결과만 적습니다 — 통과만 적힌 기록은 다음 사람이 믿을 수 없고, 그러면 기록 전체가 무게를 잃습니다.

**저장 위치** — `docs/history/{YYYY-MM-DD}-{작업이름}.md`. 이름은 브랜치(`feat/{작업이름}`)와 맞춥니다. 같은 PR에 커밋이 더 쌓이면 새 문서를 만들지 않고 이 문서를 갱신합니다.

**기록을 빠뜨리면 push가 막힙니다** — PR 플로우를 켠 하네스에서는 `docs/history/` 변경이 없는 push를 훅이 차단합니다(끄려면 `requireHistoryDoc: false`). 종료 의례를 지침으로만 두면 바쁜 실행에서 가장 먼저 생략되는 것이 기록입니다.

**번들 템플릿 6종** — history · handoff · retro · loop-spec + 설계서(design) · 배포 점검(predeploy). 하네스 구축 시 티어에 맞게(라이트: history·handoff 2종, 풀: 4종, 설계서는 설계 문답 시작 시) 프로젝트의 `docs/templates/`로 복사되고, **그 복사본이 그 프로젝트의 단일 출처**가 됩니다(프로젝트마다 양식을 조정할 수 있게).
...
**판정 방식** — 항목마다 **pass / fail / skip** 3분류이며 skip에는 사유가 필수입니다(실기기 필요 등). blocker(배포를 막는 등급) fail이 하나라도 있으면 **"배포 불가"** 로 결론냅니다. 결론은 보고의 첫 문장에 오고, 실패 항목은 "버튼 연타 시 주문이 두 번 들어간다"처럼 사용자에게 생기는 일로 풀어 씁니다.
...
**리뷰 출력** — 지적만 하는 리뷰는 소비할 수 없으므로, 항상 Before / After / Why 표로 냅니다. Why에는 위반한 원칙 이름과 함께 "라벨과 입력창이 떨어져 어느 항목의 값인지 헷갈린다"처럼 사용자에게 어떻게 보이는지를 한 줄 붙입니다.
...
**근거는 기억이 아니라 파일** — `docs/history/` 작업 기록과 이전 회고를 읽어 잘된 점과 반복되는 문제를 뽑아냅니다.
...
**기록이 먼저입니다** — 커밋 전에 `docs/history/`에 이번 작업의 기록이 있는지 확인하고, 없으면 `history` 스킬로 먼저 씁니다. 기록 없는 push는 훅이 막습니다. 훅에 걸리고 나서 쓰는 것보다 먼저 쓰는 편이 정확합니다 — 작업 맥락이 아직 살아 있기 때문입니다.

**PR 본문은 기록에서 가져옵니다** — 변경 요약·검증 결과는 `docs/history/` 문서에 이미 쓴 내용입니다. 두 번 쓰지 않습니다.
...
```mermaid
flowchart LR
    B["branch<br/>브랜치 확인"] --> W["작업 실행<br/>TDD · 경계면 QA<br/>산출물은 _workspace/"]
    W --> D["history<br/>작업 기록 1건"]
    D --> S["보고<br/>결론 → 할 일 → 근거"]
    S -. 반복 문제 감지 시 .-> T["retro<br/>회고 제안"]
    S -. 명시 요청 시 .-> P["pr<br/>커밋 · PR"]
...
2. **실행** — 선택된 실행 모드로 진행합니다. 코드 생성은 TDD로, 모듈이 완성될 때마다 경계면 QA를 돌립니다. 역할 간에 주고받는 산출물은 `_workspace/`에 쌓입니다(커밋되지 않음)
3. **기록** — 흐름을 끝까지 본 쪽이 `_workspace/` 산출물을 근거로 `docs/history/`에 **작업 1건당 문서 1개**를 남깁니다. 매 실행 필수 의례는 이것뿐입니다
4. **보고** — 채팅에는 결론 한 문장 → 당신이 할 일 → 근거 경로 순으로 짧게. 대화를 보지 못했고 코드를 모르는 사람이 읽어도 이해되게 씁니다(절대 규칙 7)

**종료 보고는 이렇게 생겼습니다** — 무엇을 했는지의 나열이 아니라, 어떻게 끝났고 당신이 무엇을 해야 하는지가 먼저 옵니다.

> 로그인 화면의 중복 제출 방지를 끝냈습니다. 단, 한글 입력 중 Enter 케이스는 실기기가 없어 확인하지 못했습니다.
>
> 할 일: 안드로이드 실기기에서 한글 입력 중 Enter를 한 번 눌러봐 주세요.
>
> | 구분 | 내용 |
> |---|---|
> | 변경 | 로그인 폼 1개, 테스트 2개 |
> | 검증 | 테스트 14/14 통과, 5연타 → 요청 1건 |
> | 기록 | `docs/history/2026-09-04-login-double-submit.md` |

**종료 의례는 작업 1건당 문서 1개입니다.** 회고(`retro`)는 같은 피드백 2회·반복 실패가 감지될 때만 제안합니다 — 소규모 작업까지 매번 회고를 도는 것은 의례 비용입니다
...
| 훅 | 언제 발동 | 무엇을 막나 |
|---|---|---|
| **`blockGitMutation`** | Bash 명령 실행 직전 | git 변경 명령 전부. 읽기 명령(`status`·`diff`·`log`)과 사용자 승인된 `switch`만 통과. `allowCommitPush`를 켜면 commit·push가 열리지만, 그때도 **Claude 작성 표기 커밋 · force push · 히스토리 재작성 · `docs/history/` 기록 없는 push**는 계속 차단 |
...
| 2 | **코드 생성 하네스는 TDD 기본** — 인수조건 = 테스트 케이스, 종료 기준에 전체 통과 포함. 테스트는 설계 문답에서 합의한 공개 경계(seam)에만 | "됐다"의 판정을 사람 감각이 아니라 기계에 맡기려고. 구현 내부에 결합된 테스트는 리팩토링마다 깨진다 |
| 3 | **작업 산출물과 기록을 구분** — 역할 간 인계물은 `_workspace/`(휘발), 보존 기록은 `docs/history/` 하나. **PR 하나 = 문서 하나** | 채팅은 사라지지만 파일은 남는다. 역할마다 기록을 남기면 같은 작업이 흩어져 어느 것이 전체인지 알 수 없다 |
| 4 | **단일 출처 문서 준수** — 설계·컨벤션 문서와 어긋나면 사용자에게 확인 | 문서와 코드가 갈라지는 것을 막으려고 |
| 5 | **QA는 경계면 교차검증 + incremental** — 존재 확인이 아니라 생산자(API)와 소비자(화면)를 동시에 읽고 데이터 모양을 비교. 모듈 완성 직후마다. 자기가 만든 것을 자기가 검증하지 않는다 | 통합 시점에 몰아서 터지는 것을 막으려고. 생성자와 검증자가 같으면 맹점을 공유한다 |
| 6 | **시크릿은 읽지도 기록하지도 않는다** — 설정값이 필요하면 키 이름만 언급 | 기록은 파일로 보존되므로(규칙 3) 기록된 시크릿은 곧 유출 |
| 7 | **출력은 읽는 사람이 이해할 수 있게** — 독자는 대화를 못 봤고 코드를 모른다고 가정. 순서는 결론 → 당신이 할 일 → 근거(실패는 결론에 먼저). 전문용어는 첫 등장 시 한 줄 풀이, 경로·수치는 표에, 긴 글보다 표·배지·다이어그램 | 어려운 글은 사용자의 결정을 늦추고 답변(특히 설계 문답)의 질을 떨어뜨린다. 정확한데 읽히지 않는 보고는 없는 보고와 같다 |

규칙 1·3·6은 지침으로만 두지 않고 위의 훅과 `settings.json`의 deny 권한으로 **기계적으로 강제**합니다.

**규칙을 늘리지 않는 것도 규칙입니다.** 규칙 하나는 그 하네스의 모든 에이전트가 매 작업 전에 지불하는 고정비입니다. 컨텍스트 절약처럼 "하네스를 만드는 쪽"의 원칙은 이 목록이 아니라 `harness` 스킬에 있습니다 — 작업하는 에이전트가 매번 읽을 이유가 없기 때문입니다.
...
**레포 파일 vs 플랫폼 메모리** — 작업 기록·설계 문서·CLAUDE.md처럼 레포가 기록하는 것은 플랫폼 메모리에 중복 저장하지 않습니다. 레포 파일은 팀 공유·감사 추적용, 메모리는 그 사용자 개인용입니다.

### 8-4. 일부러 하지 않는 것

하네스는 크게 만들수록 좋아지지 않습니다. 지시문이 늘면 모델이 그만큼 더 잘하는 게 아니라, 매 턴 지불하는 고정비가 늘고 정작 중요한 규칙이 묻힙니다. 그래서 이 플러그인은 다음을 **의도적으로 하지 않습니다.**

| 하지 않는 것 | 이유 |
|---|---|
| **같은 작업을 여러 문서로 기록** | 완료 기록은 `docs/history/` 하나입니다(PR 하나 = 문서 하나). 역할마다 기록을 남기면 같은 작업이 흩어져 어느 것이 전체인지 알 수 없습니다 |
| **작업 규모와 무관한 종료 의례** | 매번 강제되는 것은 기록 하나뿐입니다. 회고는 같은 문제가 두 번 보일 때만 제안합니다 |
| **세션 간 코드 요약 캐시** | 요약은 원문이 바뀌는 순간 거짓말이 됩니다. 틀린 캐시는 없는 캐시보다 나쁘고, 매번 신선도를 검증하면 원문을 읽는 비용에 근접합니다. 파일로 남길 것은 **재현할 수 없는 것**(결정·완결된 작업·진행 중 상태)뿐입니다 |
| **페르소나 부여** | "당신은 10년 경력의 시니어 개발자입니다" 같은 정체성 서술은 현행 모델의 행동을 바꾸지 못합니다. 에이전트 정의에 넣을 것은 입출력 계약과 책임 범위입니다 |
| **플랫폼 기능 재구현** | 프롬프트 캐시·컨텍스트 요약·스킬 지연 로딩은 이미 자동입니다. 다시 만들면 절약되는 것 없이 상시 로딩만 늘어납니다 |
| **외부 문서의 API 복사** | 플랫폼 도구의 사용법은 해당 문서가 정본입니다. 옮겨 적으면 API가 바뀌는 순간 이 저장소가 거짓말을 하게 됩니다 |

**덜어낸 자리는 기계적 강제로 채웁니다.** 산문 지침은 바쁜 실행에서 가장 먼저 생략되지만, 훅은 그렇지 않습니다 — 기록 의례를 없애는 대신 기록 하나를 훅으로 강제하는 것이 v2.0.0의 방향입니다.

### 8-5. 출력은 독자 기준 — eli5에서 배운 것

Anthropic 커뮤니티 플러그인의 `eli5` 스킬은 10줄짜리 파일 하나로 설명 품질을 바꿨습니다. 새 능력을 더한 게 아니라 **"아무것도 모르는 사람에게, 그림은 크게 글은 적게"**라는 독자 가정과 매체 제약만 고정한 것입니다. 하네스의 출력물(채팅 보고·작업 기록·인계·판정·설계 문답 질문)에 같은 원리를 절대 규칙 7로 심었습니다.

- 독자는 **이 대화를 보지 못했고 코드를 모르는 사람**이라고 가정합니다 — 그 사람이 만족하면 요청한 사용자도, 다음 세션도 만족합니다
- 순서는 항상 **결론 → 당신이 할 일 → 근거**. 실패·미검증은 결론보다 먼저 밝힙니다
- 용어·내부 명칭(훅 이름·에이전트 이름)은 첫 등장에 한 줄로 풀고, 경로·명령·수치는 산문 밖(표·목록·코드 블록)에 둡니다
- 항목 비교는 표, 판정은 배지, 흐름은 다이어그램 — 글이 길어질수록 읽히지 않습니다

상세 가이드(매체별 형식·안티패턴·Before/After·점검 질문 6개)는 `skills/harness/references/plain-output.md`에 있습니다.
...
│   ├── design/                                # 설계 문답으로 승인된 설계서 (구축의 단일 출처)
│   ├── templates/                             # 공통 템플릿 (라이트: 2종, 풀: 4종 + 설계서 — 이 사본이 단일 출처)
│   ├── history/                               # 작업 기록 — PR 하나당 하나, 누적하며 삭제하지 않음
│   └── handoff/   retro/   loops/             # 인계 · 회고 · 루프 명세
├── _workspace/                                # 역할 간 작업 산출물 (gitignore — 작업이 끝나면 버려짐)
└── CLAUDE.md                                  # 하네스 포인터(트리거 규칙) + 변경 이력 (~200줄 이내)
```

기존 `settings.json`이 있으면 **덮어쓰지 않고 병합**합니다. `docs/` 하위 기록은 감사 추적 자료라 자동으로 지우지 않습니다. `_workspace/`는 gitignore에 추가되어 커밋되지 않습니다.
...
아닙니다. `history`·`handoff`·`branch`·`fe-predeploy` 같은 스킬은 하네스 구축 없이도 바로 쓸 수 있습니다. 다만 훅에 의한 자동 차단과 템플릿 배포는 하네스를 구축해야 적용됩니다.

**Q. 구축 전 설계 문답이 너무 길게 느껴집니다. 건너뛸 수 있나요?**
"문답 생략하고 바로 진행해줘"라고 하면 라운드는 생략됩니다. 다만 최종 설계 요약 1회 확인은 남습니다 — 승인 없는 설계는 추측이기 때문입니다. 반대로 문답 중 결정이 어려우면 매 질문의 "(추천)" 선택지를 고르면 됩니다.
...
| push가 "작업 기록이 없다"고 막힌다 | 정상 동작(절대 규칙 3) — `docs/history/` 변경이 없음 | `history` 스킬로 기록을 쓰고 커밋 후 재시도. 베이스가 `dev`가 아니면 `historyBase` 지정 |
...
└── skills/
    ...
    ├── branch/ pr/ loop/            # 독립 스킬들 (각 SKILL.md)
    └── handoff/ retro/
```
...
```bash
# 회귀 테스트 77종 — 검증기 + 훅 + fe-skills 클라이언트 + 배포 전 점검
node --test \
