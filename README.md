...
현재 버전 **v1.18.0** · 스킬 11종 · 안전 훅 4종 · 회귀 테스트 75종 · MIT
...
**번들 템플릿 8종** — worklog · retro · handoff · loop-spec · digest · report.html + 설계서(design) · 배포 점검(predeploy). 하네스 구축 시 티어에 맞게(라이트: worklog·handoff 2종, 풀: 6종, 설계서는 설계 문답 시작 시) 프로젝트의 `docs/templates/`로 복사되고, **그 복사본이 그 프로젝트의 단일 출처**가 됩니다(프로젝트마다 양식을 조정할 수 있게).
...
| 8 | **출력은 읽는 사람이 이해할 수 있게** — 전문용어는 첫 등장 시 한 줄 풀이, 한 문장 하나의 내용. 질문·보고·문서 모두 | 어려운 글은 사용자의 결정을 늦추고 답변(특히 설계 문답)의 질을 떨어뜨린다 |
...
├── docs/
│   ├── harness-rules.md                       # 절대 규칙 8종 정본 (단일 출처 — 다른 파일은 포인터만)
│   ├── design/                                # 설계 문답으로 승인된 설계서 (구축의 단일 출처)
│   ├── templates/                             # 공통 템플릿 (라이트: 2종, 풀: 6종 + 설계서 — 이 사본이 단일 출처)
...
└── skills/
    ...
    ├── docs/                        # 워크로그 스킬 + 템플릿 번들 (공통 6종 + design·predeploy)
...
```bash
# 회귀 테스트 75종 — 검증기 + 훅 + 다이제스트 신선도 + 배포 전 점검
node --test \
