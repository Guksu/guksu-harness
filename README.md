...
이후 그 프로젝트에서 하는 모든 작업은 **브랜치 확인 → 작업 실행 → 기록**이라는 일정한 사이클로 돌아갑니다. 매번 강제되는 의례는 이 셋뿐이고, 회고 같은 나머지는 필요할 때만 붙습니다.
...
| 7 | **출력은 읽는 사람이 이해할 수 있게** — 전문용어는 첫 등장 시 한 줄 풀이, 한 문장 하나의 내용. 질문·보고·문서 모두 | 어려운 글은 사용자의 결정을 늦추고 답변(특히 설계 문답)의 질을 떨어뜨린다 |
