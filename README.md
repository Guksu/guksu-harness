...
| **1** | 설계 문답 → 설계 | 프로젝트를 분석한 뒤 **사용자와 반복 문답(보통 30분~1시간)으로 설계부터 확정** — 매 라운드 이전 답변을 종합해 보여주고 다음 질문 2~4개를 던지며, 승인된 설계서를 `docs/design/`에 남긴다. 이어서 실행 모드(§8-2 사다리)와 **티어(라이트/풀)** 를 고르고, 풀 티어면 에이전트를 몇 개로 나눌지 판단 | **라운드별 질문에 답하고, 최종 설계 요약을 승인** |
...
ls docs/templates/       # 문서 템플릿 (라이트: 2종, 풀: 6종 + 설계서)
...
- 신규 구축은 **설계 문답부터** 합니다 — 파일을 만들기 전에 사용자와 여러 라운드(보통 30분~1시간)의 문답으로 설계를 확정하고, 승인된 설계서(`docs/design/`)가 이후 구축의 단일 출처가 됩니다.
